    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'rpl_isupport':
                ...
                break;
            case 'rpl_yourhost':
                self.emit('status-reply', message);
                break;
            case 'rpl_created':
                self.emit('status-reply', message);
                break;
...
Client.prototype.connect = function(retryCount, callback) {
    ...
    if (self.opt.secure) {
        ...
        self.conn = tls.connect(connectionOpts, function() {
            ...
            self.emit('server-tls-protocol', self.conn.getProtocol());
            self.emit('server-tls-cipher', self.conn.getCipher());
            self.emit('server-tls-certificate', self.conn.getCertificate());
            self.emit('server-tls-peercertificate', self.conn.getPeerCertificate());
            ...
            if (self.conn.authorized ||
                ...
                if (self.opt.certExpired &&
                    ...
                    self.emit('server-tls-certificate-expired');
                }
                ...
                self.emit('server-tls-authorization-error', self.conn.authorizationError);
            }
        });
        self.conn.on("error", (error) => {
            console.log("Error is: ", error);
            self.emit('error', {message:error.message,args:[]});
        });
    } else {
